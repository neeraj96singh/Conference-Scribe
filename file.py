    ...
    while True:
        with m as source:
            ...
            try:
                ...
                if "current" in filtered_query or "today" in filtered_query:
                ...
                elif (("topic" in filtered_query and "meeting" in filtered_query) or (
                        "topic" in filtered_query and "discussion" in filtered_query)):
                    with open ("timeLists.csv", 'r') as input:
                        read = csv.reader (input)
                        for row in read:
                            v = row[0]
                            if (("topic" in v and "meeting" in v) or ("topic" in v and "discussion" in v)):
                                speak.Speak (v)

                elif ("time" in filtered_query and "meeting" in filtered_query) or (
                        "when" in filtered_query and "meeting" in filtered_query):
                    with open ("timeLists.csv", 'r') as input:
                        read = csv.reader (input)
                        for row in read:
                            v = row[0]
                    ...
                    wordList = filtered_query
                    for word in wordList:
                        if (word != "meaning" and word != "can" and word != "you" and word != "tell" and word != "me" and word != "of" and word != "for"):
